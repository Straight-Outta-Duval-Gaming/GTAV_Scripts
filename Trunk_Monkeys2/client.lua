...
-- Function to get the player's current vehicle (in or out of it)
local function GetCurrentVehicle(notificationType)
    local ped = PlayerPedId()
    local vehicle
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if GetPedInVehicleSeat(currentVehicle, -1) == ped then
            ...
            QBCore.Functions.Notify("You must be the driver to " .. notificationType .. " the monkeys.", "error")
            return nil
        end
    else
...
CreateThread(function()
    ...
    while true do
        ...
        if distance < Config.PurchaseZone.radius then
            if not inZone then
                inZone = true
                QBCore.Functions.Notify("Press E to buy monkeys.", "primary")
            end

            local vehicle = GetVehiclePedIsIn(ped, false)
            if vehicle and GetPedInVehicleSeat(vehicle, -1) == ped and GetEntitySpeed(vehicle) < 1.0 then
...
RegisterNetEvent('TrunkMonkeys:client:AttemptPurchase', function()
    local vehicle = GetCurrentVehicle("purchase")

    -- 1. Check if a valid vehicle is nearby
    if not vehicle or not IsVehicleValid(vehicle) then
        QBCore.Functions.Notify("You must be in or near a valid vehicle.", "error")
        return
    end

    QBCore.Functions.TriggerCallback('TrunkMonkeys:server:IsVehicleOwner', function(isOwner)
        if not isOwner then
...
RegisterNetEvent('TrunkMonkeys:client:ReleaseMonkeys', function()
    ...
    local vehicle = GetCurrentVehicle("release")
    ...
    if not vehicle or not IsVehicleValid(vehicle) or GetEntitySpeed(vehicle) > 1.0 then
        QBCore.Functions.Notify("You must be in or near a stationary valid vehicle.", "error")
        return
    end
