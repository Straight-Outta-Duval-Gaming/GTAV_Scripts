...
CreateThread(function()
    ...
    npc = CreatePed(4, hash, Config.NPCLocation.coords.x, Config.NPCLocation.coords.y, Config.NPCLocation.coords.z - 1, Config.NPCLocation.heading, false, true)
            ...
            if IsControlJustReleased(0, 38) then
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if not Config.DisallowedClasses[vehicleClass] then
                        TriggerServerEvent('TrunkMonkeys:server:BuyMonkeys')
                    else
...
RegisterNetEvent('TrunkMonkeys:client:SpawnMonkeys', function(vehicleNetId, otherPlayers)
    ...
    for i = 1, Config.MonkeyCount do
        CreateThread(function()
            ...
            -- Find a target
            local closestHostile, _ = GetClosestPed(GetEntityCoords(monkey), {
                ignore = { PlayerPedId() },
                hostile = true
            }, Config.MonkeySearchRadius)

            if closestHostile and closestHostile ~= 0 then
